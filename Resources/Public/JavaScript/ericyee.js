$(function() {
	...
	var resizeCallback = function () {
		//console.log("resizeCallback:  " + $(".device-xs").is(":visible") + $(".device-sm").is(":visible") + $(".device-md").is(":visible") + $(".device-lg").is(":visible"));
		...
		if ($me.length) {
			...
			$ringBody.css({top: -($me.height() * .27) + 'px'});
			$ringOfCharacters.css({fontSize: ($me.height() * .035) + 'px'});
			
			ringFrontIndex = (($(window).width() < 415) ? 52 : ($(".device-xs").is(":visible")) ? 51 : ($(".device-sm").is(":visible") ? 51 : ($(".device-md").is(":visible") ? 51 : 50)))
		}
	};
	resizeCallback();	
	...
	(function initializeRingOfCharacters() {
		...
		$('.ringWord').each(function() {
			oldStr = $(this).text();
			newStr = '';
			for (var i = 0; i < oldStr.length; i++) {
				count++;
				newStr = newStr + '<li class="ringChar">' + oldStr.charAt(i) + '</li>';
			}
	...
	var moveRing = function(event) {
		...
		/* Deprecated
		while (liCount > 0) {
			li = $('.ringChar').last().detach();
			$('.ringChar').first().before(li);
			liCount--;
		}
		*/
		li = $('.ringChar:nth-child(-n+' + String(Math.round(liCount)) + ')').detach();
